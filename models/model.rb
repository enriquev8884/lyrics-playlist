...
playlist = RSpotify::Playlist.find("spotify","5FJXhjdILmRA2z5bvz4nzf")
                                  #"avatarquique222, 4sfMcuxMs96v2AiLmSyORE"
        ...
        # fill the @songs array with new instances of the Song class
        playlist.tracks.each do |track|
            pp "#{track.name} by #{track.artists.first.name}"
        end
# puts spotify_top_tracks_playlist.track.album.images.first["url"]
